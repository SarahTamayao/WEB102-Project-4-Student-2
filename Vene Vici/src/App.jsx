...
function toSuggestionRows(text) {
  if (!text) return []
...
function Home({
  ...
  const renderSuggestionTable = () => {
    if (!suggestionRows?.length) return null

    return (
      <div className="suggestions-table-wrap">
        <h4>Suggested Schools</h4>
...
`
      ...
      const response = await ai.models.generateContent({
        ...
        contents: `${repositorySummary} related to the following query please suggest schools or universities that  have majors related to this repository and please generate in a step by step list ascendingly with your preference `,
      })

      const suggestedQuery = response.text.trim()
      setSuggestionRows(toSuggestionRows(suggestedQuery))
    } catch (err) {
